...
from app.core.models import FoodItem, Home, User
...
async def admin_page(
    ...
    current_home: Home | None = await HomeService(db).get_user_current_home(
        user
    )
...
async def admin_delete_user(
    ...
    if target_user is not None:
        owned_home: Home | None = (
            await db.execute(select(Home).where(Home.owner_id == user_id))
        ).scalar_one_or_none()
                ...
                (
                    await db.execute(
                        select(FoodItem).where(
                            FoodItem.home_id == owned_home.id
                        )
                    )
                )
                .scalars()
